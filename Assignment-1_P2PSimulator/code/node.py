...
class Node:
    ...
    def __init__(self, is_fast=1):
        ...
        self.peers = []
        self.tree = Tree()
        self.frac_cpu_power = 1  # default low cpu
        self.t_blk = np.random.randint(10, 20)
        self.txn_queue = []
        # to avoid reducdancy in forwarding in p2p network
        self.p2p_history = defaultdict(list)
    ...
    def add_history(self, node, entry):
        ...
        if len(self.p2p_history[node]) == 101:
            # FIFO pop from history
            self.p2p_history[node] = self.p2p_history[node][1:]
    ...
    def create_coinbase_txn(self, txn_time):
        ...
        txn_obj = txn(-1, self.nid, amount,
                              txn_time, is_coinbase=1)
        return txn_obj
    ...
    def create_txn(self, time_offset=0):
        ...
        """
        balances = self.tree.get_balances(
            Block.bid_to_obj[self.tree.leaf_bid].pbid)
        txn_time = time_offset + \
            sample_exponential(params.MEAN_INTERARRIVAL_TIME)
    ...
    def receive_txn(self, txn_obj, recv_time, sender):
        ...
        """
        id = txn_obj.txn_id
        if txn_obj in self.txn_queue:  # txn was received and processed before
            return
        ...
        for peer in self.peers:  # forward to peers after processing
            if (id, -1) not in self.p2p_history[peer]:
                ...
                QueueManager.insert_txn_receive_event(
                    peer_recv_time, txn_obj, peer, self)
    ...
    def receive_block(self, block, time, sender):
        ...
        for peer in self.peers:  # forward to peers after verifying and processing
            if (-1, id) not in self.p2p_history[peer]:
                ...
                QueueManager.insert_block_receive_event(
                    peer_recv_time, block, peer, self)
