...
class SelfishNode(Node):
    ...
    def create_block(self, pbid, time_offset = 0, broadcast = False):
        """
        ...
        #A Selfish Miner only includes the coinbase txn to save on propagation time
        ...
        # we will need to broadcast the private block in case of state 0'
        if broadcast:
    ...
    def receive_block(self, block, time, sender):
        ...
        if block.creator_id != self.nid:
            if lead <= 0:
                ...
                # it will create and broadcast this block directly
                self.create_block(self.private_chain[-1].bid, time, True)
        ...
        elif block.creator_id == self.nid:
